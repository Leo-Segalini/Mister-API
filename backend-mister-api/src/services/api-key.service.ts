...
export class ApiKeyService {
  ...
  /**
   * Récupère les statistiques de quota pour un utilisateur
   */
  async getUserQuotaStats(userId: string): Promise<any> {
    try {
      ...
      if (userApiKeys.length === 0) {
        // Retourner des statistiques par défaut si l'utilisateur n'a pas de clés API
        return {
          type: 'free',
          daily_limit: 1000,
          daily_used: 0,
          ...
          per_minute_remaining: 60,
          reset_time: new Date().toISOString(),
          last_reset: new Date().toISOString()
        };
      }

      // Calculer les statistiques agrégées pour toutes les clés API de l'utilisateur
      let totalDailyUsed = 0;
      let totalDailyLimit = 0;
      let totalPerMinuteUsed = 0;
      let totalPerMinuteLimit = 0;
      let userType = 'free';

      // Récupérer les statistiques d'utilisation d'aujourd'hui
      const today = new Date();
      today.setHours(0, 0, 0, 0);

      for (const apiKey of userApiKeys) {
        // Déterminer le type utilisateur (premium si au moins une clé est premium)
        if (apiKey.type === 'premium') {
          userType = 'premium';
        }

        // Ajouter les limites
        totalDailyLimit += apiKey.appels_jour;
        totalPerMinuteLimit += apiKey.appels_minute;

        // Compter les appels d'aujourd'hui pour cette clé
        const todayCalls = await this.apiLogRepository.count({
          where: {
        ...
        totalDailyUsed += todayCalls;

        // Compter les appels de la dernière minute
        const oneMinuteAgo = new Date(Date.now() - 60 * 1000);
        const lastMinuteCalls = await this.apiLogRepository.count({
          where: {
            api_key_id: apiKey.id,
            created_at: MoreThanOrEqual(oneMinuteAgo)
          }
        ...
        totalPerMinuteUsed += lastMinuteCalls;
      }

      // Calculer les appels restants
      const dailyRemaining = Math.max(0, totalDailyLimit - totalDailyUsed);
      const perMinuteRemaining = Math.max(0, totalPerMinuteLimit - totalPerMinuteUsed);
      ...
      return {
        ...
        daily_used: totalDailyUsed,
        per_minute_limit: totalPerMinuteLimit,
        per_minute_used: totalPerMinuteUsed,
      ...
      this.logger.error(`Erreur lors de la récupération des statistiques de quota utilisateur: ${error.message}`);
      throw new BadRequestException('Erreur lors de la récupération des statistiques de quota');
    }
  }
